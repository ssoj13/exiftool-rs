...
### Added
- **MakerNotes field write**: FujiFilm IFD rebuild; IFD overlay for camera vendors plus Xiaomi/Oppo/OnePlus/Vivo/Realme/Huawei/Google, Phase One/Leaf, Leica (`LEICA CAMERA AG` and headerless), Hasselblad (own tags, not Sony), DJI FLOAT tags, GoPro GPMF same-size leaves. Overlay names match each vendor parser table. Olympus Equipment/CameraSettings/ImageProcessing/FocusInfo and Pentax LensInfo/AFInfo nested IFDs overlay in place. Canon CameraSettings/ShotInfo/FileInfo/ProcessingInfo int16, AFInfo/AFInfo2/FocalLength uint16, HDRInfo/VignettingCorr2 int32s overlay in place. Sony CameraSettings/FocusInfo uint16 and Tag9405a byte fields plus int16s CorrParams overlay in place. Nikon ISOInfo/DistortInfo/HDRInfo/LocationInfo/AFInfo/AFInfo2/AFTune/FlashInfo uint16 arrays overlay in place. Nikon BarometerInfo uint32 overlay. FujiFilm AFCSettings int32u overlay plus generated AF-C Mask slices. Panasonic FaceDetect `FaceCount` overlays the first u16. Kodak `KDK`/`KDK INFO` Kodak::Main scalars and ValueConv (FNumber, ExposureTime, MonthDayCreated/TimeCreated, ExposureCompensation) overlay after the 8-byte header. Nikon encrypted overlay: ShotInfo Full-crypt and `NIKON_OFFSETS`, ColorBalance levels, LensData `LensIDNumber`, NikonCustom Settings* `undef[N]` Mask bitfields, ProcessBinaryData integer indices (`_BIN`), `CustomSettingsOffset` / Z `MenuOffset` pointer chains. JPEG/RAF `write_metadata` rewrites original APP1 TIFF so MakerNotes are not dropped.
- **Tag dump/codegen**: ExifTool Mask/`0.1` indices, integer Mask keys, ProcessBinaryData `_BIN` (FORMAT or int8u), and all conditional SubDirectory tables (Nikon ShotInfo*). `NIKON_SHOTINFO_CUSTOM_DIRS` catalogs CustomSettings blob slices plus `fields` for integer Settings (Z6III/Z8/Z9). Hand-maintained `dji`/`gopro` mods stay in `generated/mod.rs`.
