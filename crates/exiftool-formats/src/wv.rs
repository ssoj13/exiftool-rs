...
impl FormatParser for WvParser {
    ...
    fn parse(&self, reader: &mut dyn ReadSeek) -> Result<Metadata> {
        ...
        meta.exif
            .set("Audio:Codec", AttrValue::Str("WavPack".to_string()));
        ...
        meta.exif
            .set("WV:VersionString", AttrValue::Str(version_str));

        // Track number (offset 10, 1 byte)
        let track_no = header[10];
        if track_no > 0 {
            meta.exif
                .set("Audio:Track", AttrValue::UInt(track_no as u32));
        }
        ...
        if index_no > 0 {
            meta.exif
                .set("WV:IndexNumber", AttrValue::UInt(index_no as u32));
        }
        ...
        if total_samples != 0xFFFFFFFF {
            meta.exif
                .set("WV:TotalSamples", AttrValue::UInt(total_samples));
        }
        ...
        meta.exif
            .set("WV:BlockSamples", AttrValue::UInt(block_samples));
        ...
        meta.exif.set(
            "Audio:ChannelMode",
            AttrValue::Str(if is_mono { "Mono" } else { "Stereo" }.to_string()),
        );
        ...
        if is_hybrid {
            meta.exif
                .set("WV:EncodingMode", AttrValue::Str("Hybrid".to_string()));
        } else {
            meta.exif
                .set("WV:EncodingMode", AttrValue::Str("Lossless".to_string()));
        }
        ...
        if is_joint && !is_mono {
            meta.exif
                .set("WV:JointStereo", AttrValue::Str("Yes".to_string()));
        }
        ...
        if file_size > 32 {
            ...
            if reader.read_exact(&mut footer).is_ok() && &footer[0..8] == b"APETAGEX" {
                meta.exif
                    .set("WV:HasAPEv2Tag", AttrValue::Str("Yes".to_string()));
            }
        }
