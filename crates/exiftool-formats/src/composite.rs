...
fn add_image_size(meta: &mut Metadata) {
    ...
    if let (Some(w), Some(h)) = (width, height) {
        if w > 0 && h > 0 {
            meta.exif
                .set("ImageSize", AttrValue::Str(format!("{}x{}", w, h)));
        }
    }
}
...
fn add_focal_length_35efl(meta: &mut Metadata) {
    ...
    if let (Some(fl), Some(cf)) = (focal, crop_factor) {
        if fl > 0.0 && cf > 0.0 {
            let efl = fl * cf;
            meta.exif
                .set("FocalLength35efl", AttrValue::Str(format!("{:.0} mm", efl)));
        }
    }
}
...
fn add_gps_position(meta: &mut Metadata) {
    let lat = parse_gps_coord(
        meta.exif.get("GPSLatitude"),
        meta.exif.get_str("GPSLatitudeRef"),
    );
    let lon = parse_gps_coord(
        meta.exif.get("GPSLongitude"),
        meta.exif.get_str("GPSLongitudeRef"),
    );

    if let (Some(lat), Some(lon)) = (lat, lon) {
        meta.exif.set(
            "GPSPosition",
            AttrValue::Str(format!("{:.6}, {:.6}", lat, lon)),
        );
    }
}
...
fn add_gps_altitude(meta: &mut Metadata) {
    ...
    if let Some(alt) = altitude {
        ...
        meta.exif
            .set("GPSAltitudeFormatted", AttrValue::Str(display));
    }
}
...
fn add_datetime_original(meta: &mut Metadata) {
    ...
    if let Some(ref ss) = subsec {
        ...
        let ss_part = if ss_clean.is_empty() {
            ss.as_str()
        } else {
            ss_clean
        };
...
fn add_scale_factor_35efl(meta: &mut Metadata) {
    ...
    if let (Some(fl), Some(fl35)) = (fl, fl35) {
        if fl > 0.0 {
            let sf = fl35 / fl;
            meta.exif
                .set("ScaleFactor35efl", AttrValue::Str(format!("{:.1}", sf)));
        }
    }
}
...
fn add_circle_of_confusion(meta: &mut Metadata) {
    ...
    meta.exif.set(
        "CircleOfConfusion",
        AttrValue::Str(format!("{:.3} mm", coc_mm)),
    );
}
...
fn add_light_value(meta: &mut Metadata) {
    ...
    let iso = meta
        .exif
        .get_u32("ISO")
        .or_else(|| meta.exif.get_u32("RecommendedExposureIndex"))
    ...
    if let (Some(t), Some(f)) = (exposure, fnumber) {
        if t > 0.0 && f > 0.0 {
            ...
            let lv = av + tv - sv;
            meta.exif
                .set("LightValue", AttrValue::Str(format!("{:.1}", lv)));
        }
    }
}
...
mod tests {
    ...
    fn test_datetime_original_with_subsec() {
        ...
        meta.exif.set(
            "DateTimeOriginal",
            AttrValue::Str("2024:06:15 14:30:25".into()),
        );
        meta.exif
            .set("SubSecTimeOriginal", AttrValue::Str("123".into()));

        add_composite_tags(&mut meta);

        assert_eq!(
            meta.exif.get_str("DateTimeOriginalFull"),
            Some("2024:06:15 14:30:25.123")
        );
    }
    ...
    fn test_datetime_original_with_timezone() {
        ...
        meta.exif
            .set("SubSecTimeOriginal", AttrValue::Str("500".into()));
        meta.exif
            .set("OffsetTimeOriginal", AttrValue::Str("+03:00".into()));

        add_composite_tags(&mut meta);

        assert_eq!(
            meta.exif.get_str("DateTimeOriginalTZ"),
            Some("2024:06:15 14:30:25.500+03:00")
        );
    }
    ...
    fn test_scale_factor_light_value() {
        ...
        meta.exif
            .set("FocalLengthIn35mmFormat", AttrValue::UInt(80));
