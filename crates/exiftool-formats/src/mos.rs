//! MOS (Leaf RAW) format parser.
//!
//! MOS is Leaf's RAW format used in their medium format digital backs.
...
impl FormatParser for MosParser {
    ...
    fn parse(&self, reader: &mut dyn ReadSeek) -> Result<Metadata> {
        ...
        if let Some(make) = meta.exif.get_str("Make") {
            ...
            if make_upper.contains("LEAF") || make_upper.contains("PHASE ONE") {
                meta.format = "MOS";
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_format_info() {
        let parser = MosParser::new();
        assert_eq!(parser.format_name(), "MOS");
        assert!(parser.extensions().contains(&"mos"));
    }
    ...
    fn test_parse_leaf() {
        ...
        assert_eq!(meta.format, "MOS");
    }

    #[test]
    fn test_parse_phase_one() {
        let parser = MosParser::new();
        let data = make_tiff_header("Phase One");
        let mut cursor = Cursor::new(data);
        let meta = parser.parse(&mut cursor).unwrap();
        assert_eq!(meta.format, "MOS");
    }
