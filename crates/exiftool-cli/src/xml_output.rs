...
pub fn format_xml(path: &Path, m: &Metadata, filter: &[String], out: &mut String) {
    ...
    let _ = writeln!(
        out,
        "         xmlns:EXIF=\"http://ns.exiftool.org/EXIF/1.0/\""
    );
    let _ = writeln!(
        out,
        "         xmlns:XMP=\"http://ns.exiftool.org/XMP/1.0/\""
    );
    let _ = writeln!(
        out,
        "         xmlns:IPTC=\"http://ns.exiftool.org/IPTC/1.0/\""
    );
    let _ = writeln!(
        out,
        "         xmlns:ICC=\"http://ns.exiftool.org/ICC_Profile/1.0/\""
    );
    let _ = writeln!(
        out,
    ...
    if let Some(ref thumb) = m.thumbnail {
        let _ = writeln!(
            out,
            "  <File:ThumbnailSize>{}</File:ThumbnailSize>",
            thumb.len()
        );
    }
...
fn determine_namespace(tag: &str) -> &'static str {
    ...
    // ICC profile tags
    if tag.starts_with("ICC")
        || tag.starts_with("Profile")
        || matches!(
            tag,
            "ColorSpace"
                | "ColorSpaceData"
