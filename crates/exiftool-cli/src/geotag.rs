...
impl GpxTrack {
    ...
    pub fn from_file(path: &Path) -> Result<Self, String> {
        let content = fs::read_to_string(path).map_err(|e| format!("Cannot read GPX: {}", e))?;
        Self::parse(&content)
    }
    ...
    pub fn parse(xml: &str) -> Result<Self, String> {
        ...
        while i < bytes.len() {
            // Find <trkpt or <wpt
            if let Some(start) =
                find_tag_start(&xml[i..], "trkpt").or_else(|| find_tag_start(&xml[i..], "wpt"))
            {
                ...
                if let Some(end) = xml[tag_start..].find('>') {
                    ...
                    if let (Some(lat), Some(lon)) = (lat, lon) {
                        // Find closing tag
                        let close_tag = if xml[tag_start..].contains("trkpt") {
                            "</trkpt>"
                        } else {
                            "</wpt>"
                        };
                        if let Some(close_pos) = xml[tag_start..].find(close_tag) {
                            ...
                            let ele = extract_element(element, "ele").and_then(|s| s.parse().ok());
                            let time =
                                extract_element(element, "time").and_then(|s| parse_iso_time(&s));
